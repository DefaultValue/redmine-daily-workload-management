module BoardQueriesHelper
  ...
  def filters_options_for_select(query)
    ...
    query.available_filters.map do |field, field_options|
      if field_options[:type] == :relation
        ...
        group = query.is_a?(BoardIssueQuery) ? :label_relations : nil
      elsif field =~ /^cf_\d+\./
  ...
  def retrieve_query(klass=BoardIssueQuery, use_session=true)
    session_key = klass.name.underscore.to_sym


    prepared_ids_values = []
    IssueStatus.where(:is_closed => 1).ids.each do |id|
      prepared_ids_values.push("#{id}")
    end
    ...
    if params[:query_id].present?
      ...
      if session[session_key].nil?
        ...
        default_sorters = [['priority', 'desc'], ['id', 'desc']]


        @query.filters = default_filters
        @query.sort_criteria = default_sorters
      end
  ...
  def retrieve_query_from_session(klass=BoardIssueQuery)
    session_key = klass.name.underscore.to_sym
    session_data = session[session_key]

    if session_data
      if session_data[:id]
        @query = BoardIssueQuery.find_by_id(session_data[:id])
        return unless @query
      else
        @query = BoardIssueQuery.new(:name => "_", :filters => session_data[:filters], :group_by => session_data[:group_by], :column_names => session_data[:column_names], :totalable_names => session_data[:totalable_names], :sort_criteria => session[session_key][:sort])
      end
